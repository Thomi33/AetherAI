#!/usr/bin/env python3
"""Perfilado local de hardware para seleccionar un modelo Aether.

No requiere dependencias externas. Prioriza modelos con thinking + tool calling
que tengan un tamaño razonable para cada tier. La selección se usa por el
instalador y puede sobrescribirse con --model-override/--force-tier.
"""
from __future__ import annotations
import json, os, re, shutil, subprocess, sys

# tier, min_usable_gb, modelo, vision, ctx, predict, planner, chat, timeout,
# batch, keep_alive, parallel, max_loaded_models, note
CATALOGO = [
    ...
    ("HIGH", 9.0, "qwen3.5:9b", "qwen3.5:9b", 32768, 4096, 2048, 10, 200,
     256, "15m", 2, 1, "12GB+ VRAM o 32GB RAM: agente local potente"),
    ("ULTRA", 16.0, "qwen3.5:35b-a3b-q4_K_M", "qwen3.5:35b-a3b-q4_K_M",
     32768, 8192, 3072, 10, 240, 384, "30m", 2, 1,
     "24GB+ VRAM/RAM abundante: modelo grande multimodal"),
]
...
def _recommended(entry, base_threads, num_gpu):
    tier, _, model, vision, ctx, pred, planner, chat, timeout, batch, keep, parallel, max_models, note = entry
    return {
        "MODELO": model,
...
def probe():
    ...
    # Approximate usable model memory after reserving room for the OS/runtime.
